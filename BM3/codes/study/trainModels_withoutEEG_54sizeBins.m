...
% choose targets
Targets = Targets(:,7:end);

% drop rows with all 0 values or Inf values
iToKeep = boolean((~(sum(Targets.Variables)==Inf) + ~(sum(Targets.Variables)==0)) - 1);
Targets = Targets(:,iToKeep);
