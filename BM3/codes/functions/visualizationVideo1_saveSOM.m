...
function [] = visualizationVideo1_saveSOM(YEAR, MONTH, DAY, TRIAL, USER, ...
    EEG, Tobii, iSOM, jSOM)
    ...
    % define directory to save video
    directory = strcat('visuals/', pathID,'/som_plots/');

    % create directory if it does not alreay exist
    createDir(directory)
    
    % save plot
    print(strcat(directory, ID,'_som_plot_',string(iSOM), 'by', string(jSOM)), ...
        '-dpng')
    
    % save som timetable
    save(strcat('objects/',pathID, '/_Synchronized/', ID, ...
        '_Synchronized_SOMTimetable', ...
        string(iSOM), 'by', string(jSOM),'.mat'),'SOMTimetable');
    %% SAVE CL VARIABLES
    
    plotCLVars(YEAR, MONTH, DAY, TRIAL, USER, 'off')
